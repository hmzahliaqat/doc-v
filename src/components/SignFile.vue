...
import { useEmployeesApi } from '@/composables/employees.ts';
import ShareDrawer from '@/pages/signature/components/ShareDrawer.vue';
interface Props {
  file: PDF;
  index: number;
  type: MenuTab;
...
const  employees =ref([]);
const { getEmployees } = useEmployeesApi()
...
async function shareDocumentEmail(id){
  setCurrentPDF({ ...file, isUpdate: false });
  const data = {
    employee_id : id,
