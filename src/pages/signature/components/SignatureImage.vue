...
import { useImageStore, useConfigStore } from '@/stores';
...
const currentSelect = ref<ImageFile | null>(null);
...
function useImage() {
  if (currentSelect.value) emit('useImage', currentSelect.value.file_path);
  close();
}
...
function deleteImage() {
  ...
  imageStore.deleteImage(currentSelect.value.id);
...
async function readerFile(files?: FileList | null) {
  try {
    ...
    const exists = imageList.value.some(img => img.file_path === result);
    if (exists) {
      showToast({ message: t('prompt.picture_already_exists'), type: 'error' });
      return;
    }

    await imageStore.addImage(result);
...
<template>
  <signature-popup
    ...
    @use="useImage"
  >
    <ul v-if="imageList.length" class="signature-list">
      <img
      ...
      <li
        ...
        :key="image.id"
        :class="[ 'rounded-[20px] relative w-full flex justify-center cursor-pointer h-[180px]', currentSelect?.id === image.id ? 'bg-primary opacity-70' : 'bg-white' ]"
      ...
      >
        ...
        />
        <sign-icon
          v-show="currentSelect?.id === image.id"
          name="close_s"
  ...
  <sign-popup v-if="isShowImagePopup" :title="$t('add_picture')">
    <div
    ...
    <button class="btn btn-base" @click="toggleImagePopup(false)">
      {{ $t('cancel') }}
    </button>
  </sign-popup>
