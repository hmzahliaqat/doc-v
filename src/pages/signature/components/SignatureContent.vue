...
import { computed, defineAsyncComponent, ref, useTemplateRef, onBeforeMount } from 'vue';
...
const {PDFList, currentPDF } = storeToRefs(usePdfStore());
...
const shared_document_id = route.query?.shared_document_id;
const is_employee = route.query?.is_employee === 'true' || route.query?.is_employee === true;
...
async function mergeFile() {
  ...
  window.requestAnimationFrame(async () => {
    try {
      ...
      const blobToBase64Promises = compressedBlobs.map(blob =>
        blob && imageCompression.getDataUrlFromFile(blob)
      );
